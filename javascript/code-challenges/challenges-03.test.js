...
const containsAnd = (arr) => {
  const andStrings = arr.filter(andString => andString.includes('and'));
  return andStrings;
};
...
const oddValues = (arr) => {
  const odds = arr.filter(odd => (odd % 2));
  return odds;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  const newArr = arr.filter(num => !forbiddenValues.includes(num));
  return newArr;
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  const highStats = arr.filter(highStat => highStat.baseStat > minBaseStat);
  return highStats;
};
...
const getStatName = (arr, minBaseStat) => {
  const highStats = arr.filter(highStat => highStat.baseStat > minBaseStat ? highStat.stat.name : 'dog');
  return highStats;
};
