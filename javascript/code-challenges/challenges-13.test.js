...
const longestString = (arr) => {
  return arr.indexOf(arr.reduce((prev, curr) => prev.length > curr.length ? prev : curr,''));
};
...
const allHappy = (arr) => {
  let happyCount = 0;
  arr.forEach(string => string.includes(':)') ? happyCount++ : false);
  return happyCount !== arr.length ? false : true;
};
