...
const screenForNames = (arr) => {
  return arr.filter(name => (/^(M|D)(r|s)?s?\.\s\w+/).test(name));
};
...
let biggerThanLuke = (arr) => {
  return arr.filter(character => parseInt(character.mass) > parseInt(arr[0].mass) ? true : false).map(character => character.name).join(' - ');
};
...
------------------------------------------------------------------------------------------------ */
const isSecure = (url) => {
  return (/^(https:\/\/)(www.)?\w+\.com/).test(url);
};
...
const detectTicTacToeWin = (board) => {
  ...
  const helpCheck = (row1, col1, row2, col2, row3, col3) => {
    return board[row1][col1] === '' && board[row2][col2] === '' && board[row3][col3] === '' ? false : board[row1][col1] === board[row2][col2] && board[row2][col2] === board[row3][col3] ? true : false;
  };

  for(let i = 0; i < board.length; i++){
    if(helpCheck(i, 0, i, 1, i, 2) || helpCheck(0, i, 1, i, 2, i) || helpCheck(0, 0, 1, 1, 2, 2) || helpCheck(0, 2, 1, 1, 2, 0)){
      bool = true;
    }
  }
  return bool;
