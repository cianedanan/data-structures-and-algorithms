...
function transformToLis(obj) {
  ...
  return newArr.map(person => `<li>${person.join(': ')}</li>`);
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
const count = (target, input) => {
  let count = 0;
  input.flat().map(num => num === target ? count++ : false);
  return count;
};
...
const totalSum = (input) => {
  return input.flat().reduce((prev, curr) => prev + curr, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => input.map(element => element.filter(num => typeof num === 'number' ? num % 5 === 0: false)).map(element => element.map(num => Math.pow(2,num)));
